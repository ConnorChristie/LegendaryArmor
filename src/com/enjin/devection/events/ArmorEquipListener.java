package com.enjin.devection.events;
...
import com.enjin.devection.events.ArmorEquipEvent.ArmorType;
import com.enjin.devection.events.ArmorEquipEvent.EquipMethod;

public class ArmorEquipListener implements Listener
{
	HashMap<String, HashMap<ArmorType, Long>> lastEquip = new HashMap<String, HashMap<ArmorType, Long>>();
	...
	public final void onInventoryClick(final InventoryClickEvent e)
	{
		...
		if (newArmorType != null)
		{
			...
			if (e.getRawSlot() == 5 && !newArmorType.equals(ArmorType.HELMET) || e.getRawSlot() == 6 && !newArmorType.equals(ArmorType.CHESTPLATE)
					|| e.getRawSlot() == 7 && !newArmorType.equals(ArmorType.LEGGINGS) || e.getRawSlot() == 8 && !newArmorType.equals(ArmorType.BOOTS))
			{
				return;
			}
		}
			...
			if (newArmorType != null)
			{
				...
				if (newArmorType.equals(ArmorType.HELMET) && (equipping ? e.getWhoClicked().getInventory().getHelmet() == null : e.getWhoClicked().getInventory().getHelmet() != null)
						|| newArmorType.equals(ArmorType.CHESTPLATE)
								&& (equipping ? e.getWhoClicked().getInventory().getChestplate() == null : e.getWhoClicked().getInventory().getChestplate() != null)
				...
				{
					ArmorEquipEvent armorEquipEvent = new ArmorEquipEvent((Player) e.getWhoClicked(), EquipMethod.SHIFT_CLICK, newArmorType, equipping ? null : e.getCurrentItem(),
	...
	public void dispenserFireEvent(BlockDispenseEvent e)
	{
		...
		if (ArmorType.matchType(e.getItem()) != null)
		{
			...
			for (Player p : loc.getWorld().getPlayers())
			{
				...
				{
					if (p.getInventory().getHelmet() == null && type.equals(ArmorType.HELMET) || p.getInventory().getChestplate() == null && type.equals(ArmorType.CHESTPLATE)
							|| p.getInventory().getLeggings() == null && type.equals(ArmorType.LEGGINGS) || p.getInventory().getBoots() == null && type.equals(ArmorType.BOOTS))
					{
						...
						if (directionFacing == BlockFace.EAST && p.getLocation().getBlockX() != loc.getBlockX() && p.getLocation().getX() <= loc.getX() + 2.3 && p.getLocation().getX() >= loc.getX()
								|| directionFacing == BlockFace.WEST && p.getLocation().getX() >= loc.getX() - 1.3 && p.getLocation().getX() <= loc.getX()
								|| directionFacing == BlockFace.SOUTH && p.getLocation().getBlockZ() != loc.getBlockZ() && p.getLocation().getZ() <= loc.getZ() + 2.3
										&& p.getLocation().getZ() >= loc.getZ()
								|| directionFacing == BlockFace.NORTH && p.getLocation().getZ() >= loc.getZ() - 1.3 && p.getLocation().getZ() <= loc.getZ())
						{
